...
extension JDTableViewController {
    ...
    private func showErrorAnimated() {
        let alert = AlertController(type:.error, title: "Payment failed!", message: "It seems that we have not received money. You can try again ", preferredStyle: Settings.shared.preferedStyle)
        
        alert.isAnimated = true
        alert.widthRatio = 0.7
        alert.isDragEnable = Settings.shared.isDragEnabled
        alert.tapToClose = Settings.shared.dismissOuside
    ...
    private func showWriteUsPoupup() {
        ...
        let alert = AlertController(icon: UIImage(named: "write"), title: "Can We Help?", message: "Any questions or feedback? We are here to help you! ", preferredStyle: Settings.shared.preferedStyle, iconSize: 80)
    ...
    private func showAlertBottomAttached() {
        let alert = BAlertController(icon: UIImage(named: "alert"), title: "Notify Me On Orders", message: "Allow push notifications so you will know when a new order arrives ", preferredStyle: Settings.shared.preferedStyle,  iconSize: 200)
        alert.widthRatio = 0.8
        alert.isDragEnable = Settings.shared.isDragEnabled
        alert.tapToClose = Settings.shared.dismissOuside
    ...
    private func showEatenAlert() {
        let alert = CAlertController(icon: UIImage(named: "eaten"), title: "yay!!", message: "You have successfully eaten everyone's bacon", preferredStyle: Settings.shared.preferedStyle, iconSize: 150)
    ...
    private func showCookieAlert() {
        let alert = AlertController(icon: UIImage(named: "cookie"), message: "Our application uses cookies to ensure you get the best experience while you are here", preferredStyle: Settings.shared.preferedStyle, iconSize: 150)
        alert.widthRatio = 0.75
        alert.isDragEnable = Settings.shared.isDragEnabled
        alert.tapToClose = Settings.shared.dismissOuside
    ...
    private func showAccountCreatedAlert() {
        let alert = AlertController(icon: UIImage(named: "account"),title:  "Your account was created!", message: "Congratulations! You just created an account. Now you can log in or save password for later.", preferredStyle: Settings.shared.preferedStyle,iconSize: 80)
        alert.isDragEnable = Settings.shared.isDragEnabled
        alert.tapToClose = Settings.shared.dismissOuside
    ...
    private func showAppointmentAlert() {
        let alert = AlertController(icon: UIImage(named: "appointment"),title:  "You have an Appointment today!", message: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",preferredStyle: Settings.shared.preferedStyle, iconSize: 180)
        alert.isDragEnable = Settings.shared.isDragEnabled
        alert.tapToClose = Settings.shared.dismissOuside
        alert.widthRatio = 0.8
        alert.cornerRadius = 5
    ...
    private func signInAlert() {
        let alert = AlertController(title:"Sign in",message: "You can always access your contents by signig back in.",preferredStyle: Settings.shared.preferedStyle)
