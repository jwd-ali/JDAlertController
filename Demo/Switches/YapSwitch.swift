...
@IBDesignable public class YapSwitch: BaseControl {
    ...
    fileprivate func layoutInnerLayer(for bounds: CGRect) {
        ...
        shape == .rounded ?  (innerLayer.cornerRadius = isInnerHidden ? 0 :
    ...
    private func layoutOffContentLayer(for bounds: CGRect) {
        ...
        let y11 = bounds.midY - size.height / 2
        let leading = (bounds.maxX - (contentLeadingTrailingPadding + borderWidth + getThumbSize().width))/2 - size.width/2
        let x11 = !isOn ? bounds.width - size.width - leading : bounds.width
        let origin = CGPoint(x: x11, y: y11)
    ...
    private func layoutOnContentLayer(for bounds: CGRect) {
        ...
        let yValue = bounds.midY - size.height / 2
        let leading = (bounds.maxX - (contentLeadingTrailingPadding + borderWidth + getThumbSize().width))/2 - size.width/2
        let xValue = isOn ? leading : -bounds.width / 2
    ...
    final func getThumbOrigin(for width: CGFloat) -> CGPoint {
        ...
        let x11 = isOn ? bounds.width - width - inset : inset
        return CGPoint(x: x11, y: inset)
    }
...
@IBDesignable public class YapSwitchSlim: YapSwitch {
    ...
    override func layoutTrackLayer(for bounds: CGRect) {
        ...
        self.shape == .rounded ?
            (self.trackLayer.cornerRadius = self.trackLayer.bounds.height / 2) :
            (self.trackLayer.cornerRadius = bounds.height * 0.2)
    }
    ...
    override func layoutInnerLayer(for bounds: CGRect) {
        ...
        shape == .rounded ?
            (innerLayer.cornerRadius = isInnerHidden ? 0 :
                innerLayer.bounds.height / 2 - inset) :
            (innerLayer.cornerRadius = isInnerHidden ? 5 :   5)
    }
...
@IBDesignable public class YapAnimatedSwitch: YapSwitch {
    ...
    override func controlDidLoad() {
        ...
        DispatchQueue.main.async {
            ...
            self.shapeLayer.path = self.isOn ? (self.drawOnPath(self.thumbLayer.bounds)) : self.drawOffPath(self.thumbLayer.bounds)
        }
    ...
    override func stateDidChange() {
        ...
        isOn ? shapeLayer.animateShape(path: drawOnPath(thumbLayer.bounds),
                                       duration: (duration - duration/2)) :
            shapeLayer.animateShape(path: drawOffPath(thumbLayer.bounds),
                                    duration: (duration - duration/2))
...
@IBDesignable public class YapModeSwitch: YapSwitch {
    ...
    override func controlDidLoad() {
        ...
        DispatchQueue.main.async {
            self.isOn ? (self.shapeLayer.path = self.drawOffPath(self.thumbLayer.bounds)) :
                (self.shapeLayer.path = self.drawOnPath(self.thumbLayer.bounds))
        }
    ...
    override func stateDidChange() {
        ...
        super.stateDidChange()
        isOn ? shapeLayer.animateShape(path: drawOffPath(thumbLayer.bounds),
                                       duration: duration - duration/1.5 ) :
            shapeLayer.animateShape(path: drawOnPath(thumbLayer.bounds),
                                    duration: duration - duration/1.5)
...
@IBDesignable public class YapDarkAndLightModeSwitch: YapSwitch {
    ...
    override func controlDidLoad() {
        ...
        DispatchQueue.main.async {
            self.isOn ? (self.maskLayer.path = self.drawOnPath(self.thumbLayer.bounds))
                : (self.maskLayer.path = self.drawOffPath(self.thumbLayer.bounds))
        }
    ...
    override func stateDidChange() {
        ...
        super.stateDidChange()
        isOn ? maskLayer.animateShape(path: drawOnPath(thumbLayer.bounds),
                                      duration: duration ) :
            maskLayer.animateShape(path: drawOffPath(thumbLayer.bounds),
                                   duration: duration )
...
extension YapDarkAndLightModeSwitch: AnimationPath {
    ...
    func setMask(isFullSize: Bool) -> CGPath {
        ...
        let  x11 = center.x  + radius  *  cos(180.degreesToRadians)
        let  y11 = center.y  + radius  *  sin(180.degreesToRadians)

        let bezierPath = UIBezierPath()

        isFullSize ? (radius = 0) : (radius += 0 )

        bezierPath.addArc(withCenter: CGPoint(x: x11, y: y11),
                          radius: radius,
