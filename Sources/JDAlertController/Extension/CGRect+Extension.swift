...
public extension CGRect {
    func drawCheckmarkPath()-> CGPath {
        ...
        let  x1 = center.x - radius*2 + (radius ) *  cos(-45.degreesToRadians)
        let  y1 = center.y - radius*2.3 + (radius  ) *  sin(-45.degreesToRadians)
        
        let  x2 = center.x - radius*2 + (radius ) * cos(135.degreesToRadians)
        let  y2 = center.y  - radius*2.3 + (radius ) * sin(135.degreesToRadians)
        
       // bezierPath.move(to: CGPoint(x: x1, y: y1))
       
        
        let  x3 = center.x - radius*2 + (radius + radius * 0.5 ) *  cos(-175.degreesToRadians)
        let  y3 = center.y - radius*2.3 + (radius  + radius * 0.5  ) *  sin(-175.degreesToRadians)
